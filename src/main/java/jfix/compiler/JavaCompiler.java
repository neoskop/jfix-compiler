...
public class JavaCompiler {
	...
	private static final Pattern CLASSNAME_PATTERN = Pattern.compile("public\\s+class\\s+([^\\s]+)", Pattern.DOTALL | Pattern.MULTILINE);

	private final List<File> sourceFiles = new CopyOnWriteArrayList<File>();
	private ClassLoader runtimeClassLoader;
	private File tmpDirectory;

	public JavaCompiler() {
		try {
			tmpDirectory = File.createTempFile(JavaCompiler.class.getName().replace(".", "_") + "-", "");
			tmpDirectory.delete();
			tmpDirectory.mkdirs();
	...
	public Object eval(String source) {
		try {
			...
			if (matcher.find()) {
				File sourceFile = new File(tmpDirectory, matcher.group(1) + ".java");
				sourceFile.deleteOnExit();
				BufferedWriter out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(sourceFile), "UTF-8"));
				out.write(source);
				out.close();
	...
	private boolean isSourceFileModified() {
		for (File sourceFile : sourceFiles) {
			File classFile = new File(sourceFile.getAbsolutePath().replace(".java", ".class"));
	...
	private void reload() {
		List<URL> classLoaderDirectories = new ArrayList<URL>();
		for (int i = 0; i < sourceFiles.size(); i++) {
			try {
				URL sourceURL = sourceFiles.get(i).getParentFile().toURI().toURL();
		...
		runtimeClassLoader = new URLClassLoader(
				classLoaderDirectories.toArray(new URL[] {}), Thread.currentThread().getContextClassLoader());
	}
